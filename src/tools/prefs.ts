...
export function registerPrefsTools(server: McpServer): void {
  server.tool(
    "seq_prefs",
    "Show current Seq MCP preferences (output format, hidden fields, message length).",
    {},
    async () => {
  ...
  server.tool(
    "seq_prefs_update",
    "Update a Seq MCP preference. Valid keys: defaultFormat (compact|table|detail|raw), maxMessageLength (number), hideFields (comma-separated list).",
    {
      key: z.string().describe("Preference key: defaultFormat, maxMessageLength, or hideFields"),
    ...
    async (params) => {
      ...
      if (params.key === "maxMessageLength") {
        parsed = Number(params.value);
        if (isNaN(parsed as number)) throw new Error("maxMessageLength must be a number");
